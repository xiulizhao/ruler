(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.ruler) {
      window.wwload.ruler = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/ruler/ruler","css!libs/ruler/ruler"], function() {
        window.wwload.ruler = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.ruler === "wait") {
      setTimeout(function () {
  ...
  function ruler_btn($ele){

    var config = {};
    config.rulerid = $ele.attr("data-rulerid");
    $ele.find(".rulerWrapper").attr("id",config.rulerid);
    config.rulermax = eval($ele.attr("data-rulermax") || "1000");
    config.minunit = eval($ele.attr("data-minunit") || "1");
    config.unitset = eval($ele.attr("data-unitset") || "10");
    config.rulervalue = eval($ele.attr("data-rulervalue") || "1");
    config.mult = eval($ele.attr("data-mult") || "1");
    config.rulermin = eval($ele.attr("data-rulermin") || "50");


    var measureRuler2 = new MeasureRuler({
      wrapperId: config.rulerid,     //容器ID，页面中写一个DIV就行 (必须)
      max: config.rulermax,                      //刻度尺最大的刻度    (非必须，默认为1000)
      minUnit: config.minunit,                  //刻度尺最小刻度    (非必须，默认为1)
      unitSet: config.unitset,                   //刻度尺单元长度    (非必须，默认是10)
      value: config.rulervalue,                      //初始化数值       (非必须，默认为1)
      mult: config.mult,                       //刻度值倍数，默认是最小刻度值为10px，如果定mult为3则最小刻度为30px (非必须，默认为1)
      callback: function (val) {     //滑动尺子过程中的回调函数     (非必须)
        //console.log(val);
        $.wwclass.helper.updateProp($ele, "data-x-rulerdata", val);
      },    
      min: config.rulermin
    });
  ...
  function processElement($ele) {
    ...
    setTimeout(function () {
      $.wwclass.helper.updateProp($ele, "data-x-inited", true);
      $ele.find(".rulerWrapper").removeClass("hidden");
      ruler_btn($ele);
    }, 100);
